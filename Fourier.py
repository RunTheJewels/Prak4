...
def fourier(f=f,x0=0.0,x1=1.0,n=100,steps=20):
	...
	for i in xrange(1,steps+1):
		for j in xrange(0,n):
			tmp[j]=np.sin(xs[j]*i*np.math.pi/mid)*ys[j]
		b[i]=1.0/mid*simps(tmp,x=xs)
		for j in xrange(0,n):
			tmp[j]=np.cos(xs[j]*i*np.math.pi/mid)*ys[j]
	...
	for i in xrange(1,steps+1):
		for j in xrange(0,n):
			for k in xrange(i,steps+1):
				solution[k,j]+=a[i]*np.cos(xs[j]*i*np.math.pi/mid)+b[i]*np.sin(xs[j]*i*np.math.pi/mid)
	...
	anm = ani.FuncAnimation(fig, _graph_animate, frames=steps+1, interval=500, repeat=False)
	plt.show()

fourier(f=h,x0=-4.5,x1=4.5,n=100,steps=30)